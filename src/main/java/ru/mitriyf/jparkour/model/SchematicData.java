...
public class SchematicData {
    private final Map<Integer, SchematicMemberData> schematicMemberDataMap = new HashMap<>();
    ...
    private int health, foodLevel, damageBomb, exitTime, timer, x, y, z, forward, size;
    ...
    private void setupSettings(YamlConfiguration cfg) {
        ...
        y = coords.getInt("y");
        z = coords.getInt("z");
        ConfigurationSection locs = cfg.getConfigurationSection("locations");
        generateLocations(locs);
    ...
    private void generateLocations(ConfigurationSection locations) {
        spawn = toDouble(locations.getString("spawn"));
        portal = toDouble(locations.getString("portal"));
        ConfigurationSection playersSection = locations.getConfigurationSection("players");
        int id = 0;
        for (String section : playersSection.getKeys(false)) {
            ConfigurationSection playerSection = playersSection.getConfigurationSection(section);
            double[] start = toDouble(playerSection.getString("start"));
            double[] end = toDouble(playerSection.getString("end"));
            SchematicMemberData schematicMemberData = new SchematicMemberData(start, end);
            ConfigurationSection pointsSection = playerSection.getConfigurationSection("points");
            for (int i = 1; i <= pointsSection.getKeys(false).size(); i++) {
                ...
                schematicMemberData.getPoints().put(i, new SchematicPointData(this, pointSection));
            }
            generatePlayerStands(schematicMemberData, playerSection, schematicMemberData.getStands());
            schematicMemberDataMap.put(utils.formatInt(section), schematicMemberData);
            id++;
        }
        size = id;
        if (values.getMaxSize() < size) {
            values.setMaxSize(size);
        }
    }
    ...
    private void generateStands(ConfigurationSection standsSection) {
        radiusStands = standsSection.getDouble("radiusStands");
        timer = standsSection.getInt("timer");
    }

    private void generatePlayerStands(SchematicMemberData schematicMemberData, ConfigurationSection locationsSection, Map<double[], String> stands) {
        for (String s : locationsSection.getStringList("stands")) {
            String[] n = s.split(";");
        ...
        schematicMemberData.setMaxLefts(maxLefts);
        schematicMemberData.setMaxRights(stands.size() - maxLefts);
    }
    ...
    public double getAccuracy(int lefts, int rights, int maxLefts, int maxRights) {
