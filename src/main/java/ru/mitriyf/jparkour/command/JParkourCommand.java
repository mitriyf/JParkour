package ru.mitriyf.jparkour.command;
...
import ru.mitriyf.jparkour.command.subcommand.AdminEditorSubCommand;
import ru.mitriyf.jparkour.command.subcommand.PartySubCommand;
...
public class JParkourCommand implements CommandExecutor {
    ...
    public JParkourCommand(JParkour plugin) {
        ...
        partySubCommand = new PartySubCommand(plugin);
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String s, @NotNull String[] args) {
        ...
        if (!permission || args.length == 0 || args.length >= 13 || args[0].equalsIgnoreCase("help")) {
            if (permission) {
                sendMessage(sender, values.getCommandHelp());
            } else {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "party": {
                partySubCommand.checkPartySubCommand(sender, args);
                return false;
            }
            case "admin": {
    ...
    private void join(CommandSender sender, String[] args) {
        ...
        if (args.length == 2) {
            gameManager.join(p, args[1].toLowerCase(), false);
            return;
        }
        gameManager.join(p, null, false);
    }
    ...
    private void exit(CommandSender sender) {
        ...
        if (game != null) {
            ...
            if (gameName.startsWith(values.getWorldStart() + "E")) {
                if (!gameManager.getConfirmation().contains(gameName)) {
                    ...
                    gameManager.getConfirmation().add(gameName);
                    scheduler.runTaskLater(plugin, () -> gameManager.getConfirmation().remove(gameName), 600);
                    return;
                }
            }
